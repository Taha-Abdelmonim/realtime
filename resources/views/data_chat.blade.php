...
<!-- Messages -->
<div id="reset-scroll" class="flex-1 overflow-auto" style="background-color: #DAD3CC">
  <div class="py-2 px-3" id="wrapper-new-message">
      ...
      <div class="flex justify-end mb-2">
        <div class="rounded py-2 px-3" style="background-color: #E2F7CB">
          ...
          <p class="text-right text-xs text-grey-dark mt-1">
            {{ date_format($chat->updated_at, "H:i:s A") }}
          </p>
        </div>
      </div>
      @else
...
<form action="{{ route('chat.store') }}" method="post" id="send-message">
  ...
  <div class="bg-grey-lighter px-4 py-4 flex items-center">
    ...
    <div class="flex-1 mx-4">
      <input class="w-full border rounded px-2 py-2" type="text" name="message" />
    </div>
    <input type="hidden" value="{{ Auth::user()->id }}" name="send_user_id">
    <input type="hidden" value="{{ $user->id }}" name="received_user_id">
    <div class="flex items-center gap-x-2">
      <button type="submit"  class="send-button">
        <img src="{{ asset('images/message.png') }}" alt="" class="w-6" />
      </button>
